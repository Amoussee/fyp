...
export const getSchoolByName = async (req, res) => {
  ...
  try {
    const result = await pool.query('SELECT * FROM schools WHERE school_name ILIKE $1', [
      `%${name}%`,
    ]);
  ...
  try {
    const result = await pool.query(
      `
            ...
            RETURNING *
            `,
      [school_name, address, mrt_desc, dgp_code, zone_code, mainlevel_code, nature_code, type_code],
    );
