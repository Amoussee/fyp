...
export const upsertDashboard = async (req, res) => {
    ...
    if (!name || (!owner_id && !sub && !email)) {
        return res.status(400).json({ error: 'Missing required fields: name AND (owner_id OR sub OR email)' });
    }
...
export const deleteDashboard = async (req, res) => {
    const { id } = req.params;
    try {
        const result = await pool.query('DELETE FROM dashboards WHERE dashboard_id = $1 RETURNING *', [id]);
