...
const DashboardCell = ({ config, onClick, surveySchema, surveyResponses }: DashboardCellProps) => {
  ...
  const flatData = React.useMemo(() => {
    return surveyResponses.map((r) => {
      ...
      if (surveySchema?.fields) {
        surveySchema.fields.forEach((f: { label: string; id: string }) => {
          row[f.label] = (r?.responses || {})[f.id];
        });
      }
      return row;
    });
  ...
  return (
    <div className="bg-white p-2 shadow-xl rounded-2xl h-[500px] overflow-hidden relative group border border-gray-100 hover:shadow-2xl transition-all duration-300">
      <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity z-10 flex gap-2">
        <button
          onClick={onClick}
          className="bg-white/80 backdrop-blur-sm shadow-sm p-1.5 rounded-md text-xs hover:bg-indigo-50 hover:text-indigo-600 transition-all font-semibold"
        >
          Edit
        </button>
      </div>
      ...
      <div className="h-full w-full">
        <PivotTableV2 data={flatData} readOnly={true} initialState={config.pivotState} />
      </div>
    </div>
  );
};
